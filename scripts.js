...
drawBtn.addEventListener('click', () => {
    ...
    if(drawnCards.length < 20) {
      ...
      cardImgElem.alt = 'Card image cannot be displayed';
    }
    else {
  ...
  currentCard.addEventListener('click', () => {
    if(isWinning !== true){
      if(drawnCards.length == 0){
        ...
        alert("You can't pick a card that does not match what you have drawn. Please draw a card again.");
      }
      else if(currentCard.children.length < 1){
        const imgElement = document.createElement('img');
        imgElement.src = 'images/hornet.png';
        imgElement.alt = 'An image of hornet';
        currentCard.appendChild(imgElement);
        var audio = new Audio('gitgud.mp3');
        audio.play();
